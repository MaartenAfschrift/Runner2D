...
constexpr int NR = ndof + 4*3;    // # residual torques
...
int F_generic(const T** arg, T** res) {
    ...
    SpatialVec GRF_r =  GRF_s1_r + GRF_s2_r + GRF_s3_r;
    SpatialVec GRF_l = GRF_s1_l + GRF_s2_l + GRF_s3_l;
    
    // center of pressure
    // Computation Forces and moment in ground
	SpatialVec GRF_calcn_l = GRF_s1_l;
	SpatialVec GRF_toes_l = GRF_s2_l + GRF_s3_l;
	SpatialVec GRF_calcn_r = GRF_s1_r;
	SpatialVec GRF_toes_r = GRF_s2_r + GRF_s3_r;
    ...
    for (int i = 0; i < nc; ++i) {
            res[0][i + ndof + nc] = value<T>(GRF_l[1][i]); // GRF_l (x and y)
        }
    for (int i = 0; i < nc; ++i) {
            res[0][i + ndof + nc*2] = value<T>(TorqueRight[i]); // GRF_r (x and y)
        }
    for (int i = 0; i < nc; ++i) {
            res[0][i + ndof + nc*3] = value<T>(TorqueLeft[i]); // GRF_l (x and y)        }
        }
    return 0;
}
